...
NivVieCR <- (MyData$NivVie - mean(MyData$NivVie)) / sd(MyData$NivVie)
NivVieCR
# methode 1
par(mfrow=c(2,2))
...
#TEST DE CORRELATION
#  pval ] 0.1: 1] (pas significative), ] 0.05 : 0.1] (peu sig.), ] 0.01 : 0.05 ] (sig.), < 0.01 (très sig.)

#HO la correlation est egale à 0
#H1 la correlation n'est pas egale à 0
#si pvalue <0.01, l'hypothèse H0 est rejetée



#ensuite regarde ou la pvalue se situe
# lorsque pvalue ]0.1:1] (pas significatif), ]0.01:0.1](peu significatif),
# ]0.01:0.05](significatif, <0.01 (tres significatif))
#si pvalue est significatif, alors H1 est validée


cor.test(MyData$NivVie,MyData$Esp) # H0 rejetée p-value <0.1 et p-value est < 0.01 esr trés significatif
...
## b- Mod�le linaire, lm()
LM1 <- lm(NivVie ~ Esp, data=MyData)  # faire la prediction entre deux variables
LM1

# Pr�dire
P1 <- data.frame(Esp=83) # Prediction pour l'age 83 ans
predict (LM1, P1)
